...
class AvailabilityAssert:
    ...
    def assert_availability_slots_created(self, resource_id: ResourceId, time_slot: TimeSlot) -> None:
...
class TestCapabilityScheduling:
    ...
    def test_capabililty_is_found_when_capability_present_in_time_slot(
        ...
        not_found = capability_finder.find_available_capabilities(fitness_class, another_day)
    ...
    def test_schedule_multiple_capabilities_of_the_same_type(
        ...
        capability_scheduler.schedule_multiple_resources_for_period({truck1, truck2, truck3}, loading, one_day)
    ...
    def test_find_capability_ignoring_availability(
        ...
        partially_overlapping_day = TimeSlot(one_day.from_ + timedelta(hours=1), one_day.to + timedelta(hours=1))
        capability_scheduler.schedule_resource_capabilities_for_period(
            AllocatableResourceId.new_one(), [admin], one_day
        )

        on_the_exact_day = capability_finder.find_capabilities(admin_permission, one_day)
        on_different_day = capability_finder.find_capabilities(admin_permission, different_day)
        in_slot_within = capability_finder.find_capabilities(admin_permission, hour_within_day)
        in_overlapping_slot = capability_finder.find_capabilities(admin_permission, partially_overlapping_day)
    ...
    def test_finding_takes_into_account_simulations_capabilities(
        ...
        truck_capabilities = CapabilitySelector.can_perform_all_at_the_time(truck_assets)
        one_day = TimeSlot.create_daily_time_slot_at_utc(2021, 1, 1)
        truck_resource_id = AllocatableResourceId.new_one()
        capability_scheduler.schedule_resource_capabilities_for_period(truck_resource_id, [truck_capabilities], one_day)
