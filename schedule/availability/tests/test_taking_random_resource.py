...
class TestTakingRandomResource:
    ...
    def test_can_take_random_resource_from_pool(self) -> None:
        ...
        taken_1 = self._availability.block_random_available(resources_pool, one_day, owner_1)
        assert taken_1 in resources_pool
        assert self._resource_is_taken_by_owner(taken_1, owner_1, one_day)

        taken_2 = self._availability.block_random_available(resources_pool, one_day, owner_2)
        assert taken_2 in resources_pool
        assert self._resource_is_taken_by_owner(taken_2, owner_2, one_day)

        taken_3 = self._availability.block_random_available(resources_pool, one_day, owner_3)
        assert taken_3 in resources_pool
        assert self._resource_is_taken_by_owner(taken_3, owner_3, one_day)

        taken_4 = self._availability.block_random_available(resources_pool, one_day, owner_3)
    ...
    def test_nothing_is_taken_when_no_resource_in_pool(self) -> None:
        ...
        taken_1 = self._availability.block_random_available(resources, jan_1, Owner.new_one())

        assert taken_1 is None

    def _resource_is_taken_by_owner(self, resource_id: ResourceId, owner: Owner, one_day: TimeSlot) -> bool:
        resource_availability = self._availability.find(resource_id, one_day)
        return all(availability.blocked_by == owner for availability in resource_availability.resource_availabilities)
