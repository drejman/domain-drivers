...
class ScheduleBasedOnChosenResourcesAvailabilityCalculator:
    ...
    def _find_slot_for_stage(self, chosen_resources_calendars: Calendars, stage: Stage) -> TimeSlot:
        ...
        while not self._is_slot_long_enough_for_stage(stage, common_slot_for_all_resources):
    ...
    def _possible_slots(self, chosen_resources_calendars: Calendars, stage: Stage) -> list[TimeSlot]:
        result: list[TimeSlot] = []
        for resource in stage.resources:
            calendar = chosen_resources_calendars.get(resource)
            matching_slots = [
                slot for slot in calendar.available_slots() if self._is_slot_long_enough_for_stage(stage, slot)
            ]
