...
class StageAssert:
    ...
    def assert_starts_at(self, start: datetime) -> None:
        ...
        assert self._actual.from_ == start, f"Expected to start at {start}, but got {self._actual.from_}"

    def assert_with_slot(self, expected: TimeSlot) -> Self:
        __tracebackhide__ = True

        assert self._actual == expected, f"Expected to be {expected}, but got {self._actual}"
        return self

    def assert_ends_at(self, end: datetime) -> None:
        __tracebackhide__ = True

        assert self._actual.to == end, f"Expected to end at {end}, but got {self._actual.to}"
...
class StageScheduleAssert(StageAssert):
    ...
    def assert_is_before(self, stage_name: str) -> None:
        ...
        assert self._actual.to <= schedule_from, f"Expected to be before {schedule_from}, but is not ({self._actual})"
    ...
    def assert_starts_together_with(self, stage_name: str) -> None:
        ...
        assert self._actual.from_ == schedule_from, (
    ...
    def assert_is_after(self, stage_name: str) -> None:
        ...
        assert self._actual.from_ >= schedule_to, f"Expected to be after {schedule_to}, but is not ({self._actual})"
