...
from schedule.availability.calendar import Calendar
from schedule.availability.calendars import Calendars
...
class TestScheduleCalculation:
    def test_calculate_schedule_based_on_the_start_day(
        ...
        parallel_stages = ParallelStagesSequence.of(
    ...
    def test_can_adjust_to_dates_of_one_reference_stage(
        ...
        schedule = Schedule.based_on_reference_stage_time_slots(reference_stage, jan_1_5, parallel_stages)

        schedule_assert = ScheduleAssert(schedule)
        schedule_assert.assert_has_stage("S1").assert_with_slot(dec_29_jan_1).assert_is_before("S4-Reference")
        schedule_assert.assert_has_stage("S2").assert_with_slot(jan_1_11).assert_starts_together_with("S4-Reference")
        schedule_assert.assert_has_stage("S3").assert_with_slot(jan_5_7).assert_is_after("S4-Reference")
        schedule_assert.assert_has_stage("S4-Reference").assert_with_slot(jan_1_5)

    def test_no_schedule_is_calculated_if_reference_stage_to_adjust_to_does_not_exists(self, jan_1_5: TimeSlot) -> None:
        stage1 = Stage("Stage1").of_duration(timedelta(days=3))
        stage2 = Stage("Stage2").of_duration(timedelta(days=10))
        stage3 = Stage("Stage3").of_duration(timedelta(days=2))
        ...
        schedule = Schedule.based_on_reference_stage_time_slots(Stage("Stage5"), jan_1_5, parallel_stages)
    ...
    def test_adjusts_schedule_to_availability_of_needed_resources(
        ...
        stage1 = Stage("Stage1").of_duration(timedelta(days=3)).with_chosen_resource_capabilities(r1)
        stage2 = Stage("Stage2").of_duration(timedelta(days=10)).with_chosen_resource_capabilities(r2, r3)
        ...
        schedule = Schedule.based_on_chosen_resource_availability(Calendars.of(cal1, cal2, cal3), [stage1, stage2])
