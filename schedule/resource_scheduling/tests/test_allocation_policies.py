...
from schedule.resource import EmployeeId, EmployeeSummary, Seniority
...
class TestAllocationPolicies:
    def test_default_policy_returns_just_one_skill_at_once(self) -> None:
        ...
        capabilities = EmployeeAllocationPolicy.default_policy().simultaneous_capabilities_of(summary)
    ...
    def test_permissions_can_be_shared_between_projects(self) -> None:
        ...
        all_capabilities = list(itertools.chain(*[c.capabilities for c in capabilities]))
    ...
    def test_can_create_composite_policy(self) -> None:
        ...
        assert CapabilitySelector.can_perform_one_of(Capability.skills("JAVA", "PYTHON")) in capabilities
        assert capabilities.count(CapabilitySelector.can_just_perform(Capability.permission("ADMIN"))) == 3
