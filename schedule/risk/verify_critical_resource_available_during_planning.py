from schedule.availability import AvailabilityFacade, Calendar
from schedule.planning import CriticalStagePlannedEvent
...
class VerifyCriticalResourceAvailableDuringPlanning:
    ...
    def handle(self, event: CriticalStagePlannedEvent) -> None:
        if event.critical_resource_id is None:
            return

        calendar = self._availability_facade.load_calendar(event.critical_resource_id, event.stage_time_slot)
